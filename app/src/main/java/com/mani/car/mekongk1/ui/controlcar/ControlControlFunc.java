...
import com.mani.car.mekongk1.ui.controlcar.borrowcar.ActivityBorrowCar;
 ...
 */
@Deprecated
public class ControlControlFunc {
    // ========================out======================
    private static ControlControlFunc _instance;

    public static ControlControlFunc getInstance() {
        if (_instance == null)
            _instance = new ControlControlFunc();
        return _instance;
    }
    ...
    public void controlButtonPress(int func, String funName, final View parentView) {
        ...
        List<Integer> arr = ManagerControlButtons.getInstance().getBtnsIdList();
        ...
        } else if (func == arr.get(3)) {//"双闪鸣笛" 寻车
            if (!com.kulala.staticsfunc.static_system.SystemMe.isNetworkConnected(GlobalContext.getContext()) && BlueAdapter.current_blue_state < STATE_CONNECT_OK) {
                GlobalContext.popMessage("请检查是否有打开网络", GlobalContext.getContext().getResources().getColor(R.color.popTipWarning));
                return;
            }
            OCtrlCar.getInstance().ccmd1233_controlCar(currentCar, 6, 0);
        } else if (func == arr.get(8)) {//"电子围栏" 1-100km
            if (funName.equals("取消围栏")) {//不用确认，直接发
                //演示
                if (currentCar.isActive == 0) {
                    ...
                    ManagerControlButtons.getInstance().changeNoActiveArea(true);
                    return;
                }
                            ...
                            public void onClickConfirm(boolean isClickConfirm) {
                                if (isClickConfirm) {
                                    ...
                                    if (currentCar.isActive == 0) {
                                        ...
                                        ManagerControlButtons.getInstance().changeNoActiveArea(false);
                                        return;
                                    }
